...
router.get("/squad/:targetId", authMiddleware, async (req, res, next) => {
  try {
    ...
    if (myAccountId !== target.account_id) {
      ...
      return res
        .status(200)
        .json({ [`${target.account_name} 님의 스쿼드 `]: squadList });
    }
    // 내 스쿼드인 경우
    else {
      const squad = await prisma.squad.findUnique({
      ...
      if (!squad)
        return res
          .status(404)
          .json({ message: "현재 스쿼드를 보유하고 있지 않습니다." });
  ...
  async (req, res, next) => {
    try {
      ...
      if (players.length === 0)
        return res
          .status(404)
          .json({ message: "현재 보유한 선수가 존재하지 않습니다." });
      const squad = await prisma.squad.findUnique({
        where: { account_id: accountId },
      });
      if (squad.length >= 3)
        return res
          .status(400)
          .json({ message: "더이상 스쿼드를 추가할 수 없습니다." });
      ...
      if (!player)
        return res
          .status(404)
          .json({ message: "현재 해당 선수를 보유하고 있지 않습니다." });
